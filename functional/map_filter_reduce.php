...
$is_baked = function ($recipe) use ($baked) {
	...
	return array_filter($baked, function ($item) use ($recipe) {
...
# false for a recipe, it won't appear in the $backing_recipes output array.

$backing_recipes = array_filter($all_recipes, $is_baked);

print_r($backing_recipes);
...
$best_recipe = array_reduce($backing_recipes, $get_longest, '');
